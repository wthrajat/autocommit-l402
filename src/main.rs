...
async fn generate_message(
    ...
    if l402_config.enabled {
        println!("{}", "Using L402 Pay-per-Commit mode for generation".blue());
    } else {
...
async fn main() -> Result<()> {
    ...
    let yaml_cfg = config::load_yaml_config();

    let l402_config = types::L402Config {
        enabled: args.l402
            || args.lnd_host.is_some()
        ...
        lnd_host: args
            .lnd_host
            .clone()
            .or_else(|| std::env::var("LND_REST_HOST").ok())
            .or_else(|| yaml_cfg.as_ref().and_then(|y| y.lnd_host.clone())),
        lnd_macaroon: yaml_cfg.as_ref().and_then(|y| y.lnd_macaroon.clone()),
        nwc_uri: args
            .nwc_uri
            .clone()
            .or_else(|| std::env::var("NWC_CONNECTION_URI").ok())
            .or_else(|| yaml_cfg.as_ref().and_then(|y| y.nwc_uri.clone())),
        l402_proxy: args
            .l402_proxy
            .clone()
            .or_else(|| std::env::var("L402_PROXY_URL").ok())
            .or_else(|| yaml_cfg.as_ref().and_then(|y| y.l402_proxy.clone())),
    };

    if !l402_config.enabled && !config::config_file_exists() {
    ...
    let message = generate_message(model, message_style, l402_config).await?;
