...
pub fn l402_token_db_path() -> Result<PathBuf> {
    let home = std::env::var("HOME")
        .or_else(|_| std::env::var("USERPROFILE"))
        .unwrap_or_else(|_| ".".to_string());
    let dir = PathBuf::from(home).join(".config").join("autocommit-rs");
    std::fs::create_dir_all(&dir)?;
    Ok(dir.join("l402_tokens.db"))
}
...
pub fn load_yaml_config() -> Option<YamlConfig> {
